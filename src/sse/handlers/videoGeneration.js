...
async function requireValidApiKey(request) {
  ...
  return { apiKey, apiKeyOptions: await buildApiKeyOptions(apiKey) };
}
...
export async function handleVideoCreate(request, action) {
  const authResult = await requireValidApiKey(request);
  if (authResult instanceof Response) return authResult;
  const { apiKeyOptions } = authResult;
  ...
  while (true) {
    const credentials = await getProviderCredentials(provider, excludeConnectionIds, model, { ...apiKeyOptions, preferredConnectionId });
...
export async function handleVideoGet(request, requestId) {
  ...
  const credentials = await getProviderCredentials(provider, null, null, { ...apiKeyOptions, preferredConnectionId });
  const policyResponse = policyCredentialsResponse(credentials, errorResponse);
  if (policyResponse) return policyResponse;
  if (!credentials || credentials.allRateLimited) {
    return errorResponse(HTTP_STATUS.BAD_REQUEST, `No credentials for provider: ${provider}`);
