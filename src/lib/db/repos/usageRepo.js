...
export async function getApiKeyPolicyUsedTokens({ apiKey, provider, connectionIds = [], startDate = null }) {
  if (!apiKey || !provider) return 0;
  const db = await getAdapter();
  const conds = [`apiKey = ?`, `provider = ?`];
  const params = [apiKey, provider];
  if (Array.isArray(connectionIds) && connectionIds.length > 0) {
    conds.push(`connectionId IN (${connectionIds.map(() => "?").join(", ")})`);
  ...
  const row = db.get(
    `SELECT COALESCE(SUM(COALESCE(promptTokens, 0) + COALESCE(completionTokens, 0)), 0) AS used
     FROM usageHistory
