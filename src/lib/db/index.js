...
export {
  ...
  getOtherApiKeyPolicies, updateApiKeyQoderCreditUsageLedger, recordApiKeyQoderCreditUsage, resetApiKeyQoderCreditUsage,
...
export {
  ...
  appendRequestLog, getRecentLogs, getApiKeyPolicyUsedTokens,
...
export async function exportDb() {
  ...
  const out = {
    ...
    apiKeys: db.all(`SELECT * FROM apiKeys`).map((r) => ({ id: r.id, key: r.key, name: r.name, machineId: r.machineId, isActive: r.isActive === 1, policy: parseJson(r.policy, null), createdAt: r.createdAt })),
  ...
  db.transaction(() => {
    ...
    for (const k of payload.apiKeys || []) {
      db.run(
        ...
        [k.id, k.key, k.name || null, k.machineId || null, k.isActive === false ? 0 : 1, stringifyJson(k.policy || null), k.createdAt || new Date().toISOString()]
      );
    }
