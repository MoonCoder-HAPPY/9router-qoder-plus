...
async function validateGeminiNativeClientKey(request) {
  ...
  const valid = apiKey ? await isValidApiKey(apiKey) : true;
  if (!valid) {
    return Response.json({ error: { message: "Invalid API key" } }, { status: 401 });
  }

  return { apiKeyOptions: await buildApiKeyOptions(apiKey) };
}
...
async function forwardGeminiNativeRequest(request, body, model, action) {
  const authResult = await validateGeminiNativeClientKey(request);
  ...
  while (true) {
    const credentials = await getProviderCredentials("gemini", excludeConnectionIds, modelId, apiKeyOptions);
    const policyResponse = policyCredentialsResponse(
      credentials,
