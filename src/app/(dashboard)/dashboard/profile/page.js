...
const DEFAULT_MODEL_IDLE_ALERT = {
  enabled: false,
  idleMinutes: 5,
  cooldownMinutes: 5,
  dingtalkWebhook: "",
  dingtalkSecret: "",
  quotaUsageThresholdEnabled: false,
...
export default function ProfilePage() {
  ...
  const saveModelIdleAlert = async (e) => {
    e?.preventDefault();
    setIdleAlertLoading(true);
    setIdleAlertStatus({ type: "", message: "" });

    const payload = {
      enabled: idleAlertForm.enabled === true,
      idleMinutes: Math.max(1, Number(idleAlertForm.idleMinutes) || 5),
      cooldownMinutes: Math.max(0, Number(idleAlertForm.cooldownMinutes) || 0),
      dingtalkWebhook: idleAlertForm.dingtalkWebhook.trim(),
      dingtalkSecret: idleAlertForm.dingtalkSecret.trim(),
      quotaUsageThresholdEnabled: idleAlertForm.quotaUsageThresholdEnabled === true,
      quotaUsageThresholdPercent: Math.min(100, Math.max(1, Number(idleAlertForm.quotaUsageThresholdPercent) || 80)),
      locale,
      messageTemplate: idleAlertForm.messageTemplate.trim() || DEFAULT_MODEL_IDLE_ALERT.messageTemplate,
    };
    ...
    try {
      const res = await fetch("/api/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ modelIdleAlert: payload }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setIdleAlertStatus({ type: "error", message: data.error || translate("Failed to save alert settings") });
        return false;
      }
      setSettings((prev) => ({ ...prev, ...data }));
      setIdleAlertForm(normalizeIdleAlertForm(data?.modelIdleAlert));
      setIdleAlertStatus({ type: "success", message: translate("Model idle alert settings saved") });
      return true;
    } catch (err) {
      setIdleAlertStatus({ type: "error", message: translate("An error occurred") });
      return false;
    } finally {
      setIdleAlertLoading(false);
    }
  };

  const testModelIdleAlert = async () => {
    setIdleAlertTestLoading(true);
    setIdleAlertStatus({ type: "", message: "" });
    try {
      const saveEvent = { preventDefault() {} };
      const saved = await saveModelIdleAlert(saveEvent);
      if (!saved) return;
      const res = await fetch("/api/settings/model-idle-alert/test", { method: "POST" });
      const data = await res.json().catch(() => ({}));
      if (res.ok && data?.ok) {
        setIdleAlertStatus({ type: "success", message: translate("DingTalk test message sent") });
      } else {
        setIdleAlertStatus({ type: "error", message: data?.error || translate("DingTalk test failed") });
      }
    } catch (err) {
      setIdleAlertStatus({ type: "error", message: translate("An error occurred") });
    } finally {
      setIdleAlertTestLoading(false);
    }
  };
      ...
      <div className="flex flex-col gap-6">
        ...
        <Card>
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-base sm:text-lg font-semibold">Model Idle Alert</h3>
          </div>

          <form onSubmit={saveModelIdleAlert} className="flex flex-col gap-4">
            <div className="flex items-start sm:items-center justify-between gap-4">
              <div className="flex-1 min-w-0">
                <p className="font-medium text-sm sm:text-base">DingTalk Alert</p>
                <p className="text-xs sm:text-sm text-text-muted">
                  Send one DingTalk message when successful model calls stay idle past the threshold.
                </p>
              </div>
              <Toggle
                checked={idleAlertForm.enabled === true}
                onChange={() => updateIdleAlertForm("enabled", !(idleAlertForm.enabled === true))}
                disabled={loading || idleAlertLoading}
              />
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-border/50">
              <div className="flex flex-col gap-2">
                <label className="font-medium text-sm sm:text-base">Idle Minutes</label>
                <Input
                  type="number"
                  min="1"
                  step="1"
                  value={idleAlertForm.idleMinutes}
                  onChange={(e) => updateIdleAlertForm("idleMinutes", e.target.value)}
                  disabled={loading || idleAlertLoading}
                />
              </div>
              <div className="flex flex-col gap-2">
                <label className="font-medium text-sm sm:text-base">Alert Cooldown</label>
                <Input
                  type="number"
                  min="0"
                  step="1"
                  value={idleAlertForm.cooldownMinutes}
                  onChange={(e) => updateIdleAlertForm("cooldownMinutes", e.target.value)}
                  disabled={loading || idleAlertLoading}
                />
                <p className="text-xs sm:text-sm text-text-muted">Minutes to wait before another idle alert can be sent.</p>
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-border/50 flex flex-col gap-4">
              <div className="flex items-start sm:items-center justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-sm sm:text-base">API Key Usage Threshold</p>
                  <p className="text-xs sm:text-sm text-text-muted">
                    Send a DingTalk message when any API key uses at least this percentage of its assigned quota.
                  </p>
                </div>
                <Toggle
                  checked={idleAlertForm.quotaUsageThresholdEnabled === true}
                  onChange={() => updateIdleAlertForm("quotaUsageThresholdEnabled", !(idleAlertForm.quotaUsageThresholdEnabled === true))}
                  disabled={loading || idleAlertLoading}
                />
              </div>
              <div className="flex flex-col gap-2 max-w-xs">
                <label className="font-medium text-sm sm:text-base">Usage Threshold Percent</label>
                <Input
                  type="number"
                  min="1"
                  max="100"
                  step="1"
                  value={idleAlertForm.quotaUsageThresholdPercent}
                  onChange={(e) => updateIdleAlertForm("quotaUsageThresholdPercent", e.target.value)}
                  disabled={loading || idleAlertLoading || idleAlertForm.quotaUsageThresholdEnabled !== true}
                />
              </div>
            </div>

            <div className="flex flex-col gap-2">
              <label className="font-medium text-sm sm:text-base">DingTalk Webhook</label>
              <Input
                placeholder="https://oapi.dingtalk.com/robot/send?access_token=..."
                value={idleAlertForm.dingtalkWebhook}
                onChange={(e) => updateIdleAlertForm("dingtalkWebhook", e.target.value)}
                disabled={loading || idleAlertLoading}
              />
            </div>

            <div className="flex flex-col gap-2">
              <label className="font-medium text-sm sm:text-base">DingTalk Secret</label>
              <Input
                type="password"
                placeholder={settings.modelIdleAlert?.hasDingtalkSecret ? translate("Leave blank to keep existing secret") : "SEC..."}
                value={idleAlertForm.dingtalkSecret}
                onChange={(e) => updateIdleAlertForm("dingtalkSecret", e.target.value)}
                disabled={loading || idleAlertLoading}
              />
              <p className="text-xs sm:text-sm text-text-muted">This value is write-only after saving.</p>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <label className="font-medium text-sm sm:text-base">Message Template</label>
              <textarea
                value={idleAlertForm.messageTemplate}
                onChange={(e) => updateIdleAlertForm("messageTemplate", e.target.value)}
                disabled={loading || idleAlertLoading}
                rows={3}
                className={cn(
                  "w-full py-2.5 px-3 text-[16px] sm:text-sm text-text-main bg-surface-2 rounded-[10px]",
                  "border border-transparent placeholder-text-muted/70 resize-y min-h-24",
                  "focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500/40",
                  "transition-all duration-150 ease-out disabled:opacity-50 disabled:cursor-not-allowed"
                )}
              />
              <p className="text-xs sm:text-sm text-text-muted">
                Available placeholders: {"{idleMinutes}"}, {"{lastCallAt}"}, {"{now}"}.
              </p>
            </div>

            <div className="pt-2 border-t border-border/50 flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
              <Button type="submit" variant="primary" loading={idleAlertLoading} className="w-full sm:w-auto">
                Save alert settings
              </Button>
              <Button
                type="button"
                variant="secondary"
                loading={idleAlertTestLoading}
                disabled={loading || idleAlertLoading}
                onClick={testModelIdleAlert}
                className="w-full sm:w-auto"
              >
                Test DingTalk
              </Button>
            ...
            {idleAlertStatus.message && (
              <p className={`text-xs sm:text-sm ${idleAlertStatus.type === "error" ? "text-red-500" : "text-green-500"}`}>
                {idleAlertStatus.message}
              </p>
            )}
          </form>
